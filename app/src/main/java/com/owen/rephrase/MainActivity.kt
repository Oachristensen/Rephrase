...
class MainActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == PROMPTSPEECH && resultCode == RESULT_OK) {
            if (data != null) {
                ...
                speechTextView.text = (pastText + "\n" + timerTextView.text + "  " + data.getStringArrayListExtra(RecognizerIntent.EXTRA_RESULTS)!!.get(0))
            }
        }
        if (requestCode == PICKAUDIOFILE && resultCode == RESULT_OK) {
            if (data != null) {
                data?.data?.let { selectedUri ->
                    val selectedFilePath = getFilePathFromUri(selectedUri)
    ...
    private fun getFilePathFromUri(uri: Uri): String {
        println(uri)
        //High Chance this does not work
        //TODO(Format This)
        return uri.toString()
    }
